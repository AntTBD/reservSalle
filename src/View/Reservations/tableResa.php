...
<div class="table-responsive">
    <table class="table table-bordered" id="table_resa">
        <thead class="thead-dark" style="table-layout:fixed; text-align:center;";  >
        <div id =test >
            <tr>
        ...
        foreach ($creneaux as $creneau){                //On affiche tout les creneaux (les lignes)
            echo "<tr class='text-center'><th scope='row' class='table-dark'>".$creneau->getHeureDebut()."</th>";
            ...
            foreach ($salles as $salle){            //On parcours toutes les salles (les colomnes)
                ...
                if($salleValid == 1){               //La salle est dispo
                    ...
                    if($verif == 3){
                        echo "<td style='vertical-align: middle'><span style='color: red'><b>Plein</b><br><button type='button' class='btn btn-warning'>Déjà réservé</button></td>";
                    }elseif($verif == 2){
                        echo "<td style='vertical-align: middle'>Disponible<br><button type='button' class='btn btn-warning'>Déjà réservé</button></td>";
                    }elseif($verif == 1 ){                //Il reste des places ! on peut reserver
                        $idSalle = $salle->getId();
                        $idCreneau = $creneau->getId();
                        $idUser = $_SESSION["id"];
                        echo "<td style='vertical-align: middle'>Disponible<br><button type='button' class='btn btn-success' onclick='reservation($idSalle,$idCreneau,$idUser)'>Réserver</button></td>";
                    }else{                          //Aucune place dispo on ne peux pas reserver
                        echo "<td style='vertical-align: middle'><span style='color: red'><b>Plein</b></span><br><button type='button' class='btn btn-secondary' disabled>Réserver</button></td>";
                    }
                }else{
                    echo "<td style='vertical-align: middle'>Indisponible<br><button type='button' class='btn btn-secondary' disabled>Réserver</button></td>";
                }
